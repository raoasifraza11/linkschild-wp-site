            ...
            public function _enqueue_output() {
                        ...
                        foreach ( $section['fields'] as $fieldk => $field ) {
                            if ( isset ( $field['type'] ) && $field['type'] != "callback" ) {
                                ...
                                if ( ! empty ( $this->options[ $field['id'] ] ) && class_exists( $field_class ) && method_exists( $field_class, 'output' ) && $this->_can_output_css( $field ) ) {
                                    ...
                                    if ( ( ( isset ( $field['output'] ) && ! empty ( $field['output'] ) ) || ( isset ( $field['compiler'] ) && ! empty ( $field['compiler'] ) ) || $field['type'] == "typography" || $field['type'] == "custom_typography" || $field['type'] == "icon_select" ) ) {
                                        $enqueue->output();
                                    }
                                    /* \ WGL CUSTOM TYPOGRAPHY FIX */
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if (class_exists( 'ReduxFramework_custom_typography' )){
                        $typography = new ReduxFramework_custom_typography ( null, null, $this );
                        $google_webfont = $typography->makeGoogleWebfontString2 ( $this->typography );
                    } else{
                        $typography = new ReduxFramework_typography ( null, null, $this );
                        $google_webfont = $typography->makeGoogleWebfontString ( $this->typography );
                    }
                    /* \ WGL CUSTOM TYPOGRAPHY FIX */
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        <script>
                            ...
                            /* WGL CUSTOM TYPOGRAPHY FIX */
                            WebFontConfig['google'] = {families: [<?php echo $google_webfont ?>]};
                            /* \ WGL CUSTOM TYPOGRAPHY FIX */
