    ...
    class ReduxFramework_wbc_importer {
        ...
        function __construct( $field = array(), $value ='', $parent ) {
            ...
            if ( empty( $this->extension_dir ) ) {
                ...
                if (str_replace( 'wp-content', '', WP_CONTENT_DIR ) == ABSPATH) {
                    $this->extension_url = site_url( str_replace( trailingslashit( str_replace( '\\', '/', ABSPATH ) ), '', $this->extension_dir ) );
                } else {
                    $dir_plugin = str_replace( '/plugins', '', WP_PLUGIN_DIR );
                    $this->extension_url = trailingslashit( str_replace( '\\', '/', WP_CONTENT_URL ) ) . str_replace( trailingslashit( str_replace( '\\', '/', $dir_plugin ) ), '', $this->extension_dir );
                }
            }
        }
